...
var weather = [
	{ type: 'snow', name: 'Nevando'}, 
	{ type: 'wind', name: 'Nublado'}, 
	{ type: 'rain', name: 'Tempo chuvoso'}, 
	{ type: 'thunder', name: 'Chuvas com trovoadas'},
	{ type: 'sun', name: 'Céu limpo'},
	{ type: 'empty', name: ''},
];
...
const api = {
	key: '148d9f7c85c987c9496b164cef49ce2f',
  ...
  function displayResults(weatherAPI) {
	...
	$('#city').html(`<div id="city">${cityAPI.name}/${cityAPI.country}</div>`)
	

	let now = new Date();
	date.html(`<div id="date">${dateBuilder(now)}</div>`)
	
	let temp = Math.round(weatherAPI.main.temp)
	$('.temp').html(`<div class="temp">${temp}<span>c</span></div>`)
	
	summary.html(`<div id="summary">${weatherAPI.weather[0].main}</div>`)

	let hilow = `${Math.round(weatherAPI.main.temp_min)}°c / ${Math.round(weatherAPI.main.temp_max)}°c`
	$('#hi-low').html(`<div id="hi-low">${hilow}</div>`)

	let weatherType = weatherAPI.weather[0].main
	let weatherDesc = weatherAPI.weather[0].description
	...
	switch (weatherType) {
		...
		case 'Clouds':
			...
			if(weatherDesc == 'broken clouds' || weatherDesc == 'overcast clouds'){
